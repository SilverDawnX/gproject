...
public class MenuServiceImpl implements MenuService {
    ...
    public void deleteMenu(Long id) {
        ...
        // 获取所有需要删除的菜单 ID（包含子菜单）
        Set<Long> deleteIds = new HashSet<>();
        findAllChildMenuIds(id, allMenus, deleteIds);

        // 批量删除
        menuMapper.deleteBatchIds(deleteIds);
        // 删除授予给角色的权限
        deleteIds.forEach(menuId -> permissionService.processMenuDeleted(menuId));
    }
     ...
     */
    private void findAllChildMenuIds(Long parentId, List<MenuDO> allMenus, Set<Long> resultIds) {
        resultIds.add(parentId);
        for (MenuDO menu : allMenus) {
            // 排除已处理的节点，防止死循环
            if (resultIds.contains(menu.getId())) {
                continue;
            }
            if (Objects.equals(menu.getParentId(), parentId)) {
                findAllChildMenuIds(menu.getId(), allMenus, resultIds);
            }
        }
    }
